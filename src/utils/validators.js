...
export const validateTimeSpan = (value, field) => {
  ...
  for (const row of value) {
    ...
    errors.push({
      start: validateString(
        start.format ? start.format() : start,
        "Starts at",
        {
          required: true,
        }
      ...
      duration: {
        days: validateNumber(days, "Days", { required: false, min: 0 }),
        minutes: validateNumber(minutes, "Minutes", {
          required: false,
          min: -1,
        }),
        hours: validateNumber(hours, "Hours", {
          required: false,
          min: -1,
        }),
      },
  ...
  if (R.find(isFormValid, R.pluck("duration"), errors)) {
    return errors;
  }

  if (R.find(isFormValid, R.pluck("start"), errors)) {
    return errors;
  }
};
